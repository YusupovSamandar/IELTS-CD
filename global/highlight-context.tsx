...
interface HighlightContextType {
  ...
  setHighlights: React.Dispatch<React.SetStateAction<Map<string, HighlightRange[]>>>;
...
export const HighlightProvider = ({ children }: HighlightProviderProps) => {
  const [highlights, setHighlights] = useState<Map<string, HighlightRange[]>>(new Map());

  const addHighlight = useCallback((elementId: string, highlight: HighlightRange) => {
    setHighlights(prev => {
  ...
  const removeHighlight = useCallback((elementId: string, highlightId: string) => {
    setHighlights(prev => {
      const newMap = new Map(prev);
      const existing = newMap.get(elementId) || [];
