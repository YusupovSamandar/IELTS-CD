...
export const updateIdentifyInfo = async ({
  ...
  revalidatePath(
    `/assessments/${identifyInfo.question.assessmentId}`,
    'layout'
  );
  revalidatePath(`/assessments/${identifyInfo.question.assessmentId}`, 'page');
  revalidateTag(`assessment-${identifyInfo.question.assessmentId}`);
